...
export function CurrentUserRank({ row }: CurrentUserRankProps) {
  return (
    <div className="sticky bottom-4 z-10 mt-6 rounded-xl border border-quiz-orange/30 bg-background/80 p-3 shadow-lg backdrop-blur-md">
      <div className="flex items-center gap-3">
        ...
        {row.username ? (
          <Link
            href={`/u/${row.username}`}
            className="rounded-lg bg-quiz-orange/10 px-3 py-1.5 text-xs font-semibold text-quiz-orange transition-colors hover:bg-quiz-orange/20"
          >
            View profile
          </Link>
        ) : null}
